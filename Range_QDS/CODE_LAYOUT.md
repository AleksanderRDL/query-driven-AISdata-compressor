...
| --- | --- | --- |
| `orchestration/experiment_pipeline.py` | Still coordinates end-to-end stage order and owns dump assembly, result writing, and simplified CSV export. It is no longer the owner of target prep, retained-mask freezing, evaluation mechanics, selection causality, or final summary assembly. | Keep `run_experiment_pipeline` as the stage orchestrator. Next extract artifact export/dump assembly only if tests preserve payload field names exactly. |
| `orchestration/evaluation_stage.py` | Owns matched evaluation, ablation evaluation, learned-fill diagnostics, compression audit evaluation, and shift evaluation for a single run. | Keep it limited to evaluation-stage mechanics; do not move final summary gates or artifact writing here. |
