...
def _training_target_diagnostics(
    *,
    ...
    if target_mode in QUERY_USEFUL_V1_TARGET_MODES:
        diagnostics.update(
            {
                "target_family": "QueryUsefulV1Factorized",
                "final_success_allowed": target_mode == QUERY_USEFUL_V1_FACTORIZED_TARGET_MODE,
            }
        )
...
def _discriminative_sample(
    ...
    """Return top+bottom-quantile subsample for more reliable rank correlation. See learning/README.md for details.
...
def _kendall_tau(predictions: torch.Tensor, targets: torch.Tensor) -> float:
    """Compute Kendall tau for small vectors without external deps. See learning/README.md for details."""
